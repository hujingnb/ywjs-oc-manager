...
| 数据 | 存储位置 | 是否必须备份 | 备注 |
|---|---|---|---|
| manager 业务库 | postgres 卷 / RDS | ✅ 关键 | 含组织 / 成员 / 应用 / 审计 / 任务 / refresh_tokens |
| Redis 任务队列 | redis 卷 / 托管 Redis | 📋 可选 | scheduler 会从 jobs 表重建，丢一个调度周期后自愈 |
...
docker compose -f deploy/docker-compose.prod.yml exec manager-postgres \
  pg_dump -U ocm -Fc ocm > /backups/ocm-$(date +%Y%m%d).dump
...
```sh
docker compose -f deploy/docker-compose.prod.yml exec -T manager-postgres \
  pg_restore -U ocm -d ocm --clean --if-exists < /backups/ocm-20260601.dump
```
...
```sh
docker compose -f deploy/docker-compose.prod.yml exec manager-redis \
  redis-cli BGSAVE
docker cp manager-redis:/data/dump.rdb /backups/redis-$(date +%Y%m%d).rdb
```
...
3. 启一个测试 agent，用 manager 重新颁发的 bootstrap_token 注册。
