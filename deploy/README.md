...
4. **manager 端注册**：
   - 在 `/runtime-nodes` 页点"注册节点"，填节点名、agent 访问地址、CA 与 bearer token。
   - 把 bearer token 写入该节点的 `config/agent.yaml` 的 `agent.token` 字段。
...
3. 在 manager 端依次为两个 agent 创建节点（POST `/api/v1/runtime-nodes` 拿
   bootstrap_token），调 `POST /api/v1/agent/register` 完成注册。把响应里的
   `node_id` / `agent_token` 各自写入：

   - agent-1：`config/agent.yaml` 的 `manager:` 段三字段
   - agent-2：`.local/data/agent-2-config/agent.yaml` 的 `manager:` 段三字段

4. 重启对应 agent 容器（或 force-recreate）让 v1.0.1 心跳 ticker 启动；30s 内
   manager `/runtime-nodes` 列表两个节点的 `last_heartbeat_at` 应都在持续刷新。

5. 演练自愈：`docker network disconnect oc-manager_default oc-runtime-agent-2`
