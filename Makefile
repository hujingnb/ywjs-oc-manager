...
# k3d 节点对本地 registry 域名的 DNS 解析限制并命中 IfNotPresent 镜像缓存。
override LOCAL_HERMES_IMAGE := $(K3D_REGISTRY_HOST)/oc-manager-aigowork:$(HERMES_VERSION)-dev1
...
local-reset: local-down ## 清空所有本地数据并重跑 local-up（全新重建到可用，含配置与演示数据初始化）
...
local-build: local-build-ops local-build-runtime local-build-aicc-runtime ## 构建 manager-api/web 与普通、客服 runtime 镜像推 k3d registry 并滚动重启
...
local-build-runtime: hermes-inject-contract
	# 即使 Docker 构建失败也清理注入工件，避免共享契约残留进 variant 工作区。
	status=0; \
	docker build $(HERMES_BUILD_FLAGS) \
	  -t $(LOCAL_HERMES_IMAGE) \
	...
	# 普通实例镜像使用版本 tag；仍直接导入节点，避免 app Pod 经不可解析的 registry 域名拉取。
	docker save --platform linux/amd64 $(LOCAL_HERMES_IMAGE) | docker exec -i k3d-$(K3D_CLUSTER)-server-0 ctr images import -
...
local-up: cluster-create local-build ## 一键拉起本地全栈（建集群→构建镜像→部署→配置模型→初始化演示数据）
...
# local-seed-demo：通过 manager 正式 API 幂等补齐本地演示数据并等待运行时就绪。
...
test: test-hermes-version-guard test-local-build-runtime-contract ## 跑 Hermes 构建契约和 Go 单元测试 (go test ./...)
	go test ./...

test-hermes-version-guard: ## 跑普通 Hermes 上游 ref 版本守卫回归测试
	./scripts/hermes-version-guard_test.sh

test-local-build-runtime-contract:
	./scripts/local-build-runtime-contract_test.sh
