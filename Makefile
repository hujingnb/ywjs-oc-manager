...
# 本地 embedding 服务 Ollama 的上游镜像。官方多架构镜像经 daocloud DNS 在 k3d
# 节点内解析不到、`k3d image import` 对多架构 tar 也会失败，故由宿主 docker 拉取后
# 推入 k3d 本地 registry（push 自动提取当前平台单架构），节点再从 registry 拉。
OLLAMA_UPSTREAM_IMAGE ?= ollama/ollama:latest
...
local-down: ## 删除 k3d 集群（有状态数据在 .k3d-data 固定目录保留，下次 local-up 复用；彻底清空用 local-reset）
	...
	@echo "    （mysql/redis/minio/elasticsearch/ollama），下次 make local-up 复用、数据不丢；"
...
.local-ollama-image: # 内部：确保 Ollama 镜像已在 k3d registry（宿主 pull→tag→push，幂等），仅由 local-up 调用
	# 仅当 registry 内已无 ollama:latest 时才拉取推送：节点拉的是
	# $(K3D_REGISTRY_HOST)/ollama:latest，而非官方 ollama/ollama，所以集群重建
	# （registry 数据非持久）后此镜像会丢失，需在 apply ollama.yaml 前补齐，
	# 否则 pod 因 registry 缺镜像回退原始引用做 DNS 解析失败 → ImagePullBackOff。
	@if curl -fsS http://localhost:$(K3D_REGISTRY_PORT)/v2/ollama/tags/list 2>/dev/null | grep -q '"latest"'; then \
		echo "✅ Ollama 镜像已在 registry，跳过 pull/push"; \
	else \
		echo "⏳ registry 缺 Ollama 镜像，宿主拉取并推送…"; \
		docker pull $(OLLAMA_UPSTREAM_IMAGE); \
		docker tag $(OLLAMA_UPSTREAM_IMAGE) $(K3D_REGISTRY_HOST)/ollama:latest; \
		docker push $(K3D_REGISTRY_HOST)/ollama:latest; \
		echo "✅ Ollama 镜像已推入 registry"; \
	fi
...
local-up: cluster-create local-build ## 一键拉起本地全栈（建集群→构建镜像→部署→建桶→种子管理员）
	...
	# 3) 建 MinIO bucket
	$(MAKE) local-mc-init
	# 3.1) 确保本地 embedding 用的 Ollama 镜像已在 k3d registry（缺则宿主 pull→push）
	$(MAKE) .local-ollama-image
