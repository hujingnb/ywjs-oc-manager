...
test('登录页切换到中文后 localStorage ocm.locale 写入 zh', async ({ page }) => {
  ...
  await page.locator('.n-dropdown-option', { hasText: '简体中文' }).click()
...
test('app locale PATCH 后实例语言更新为 en', async ({ page }) => {
  ...
  // 平台管理员登录。注意：manager 用 Bearer token（localStorage.ocm.access_token）+
  // CSRF double-submit（csrf_token cookie 复制到 X-CSRF-Token）做鉴权；page.request 不带
  // 这两个 header，故写操作必须在页面上下文里用 fetch 发起（自动带 cookie + 取 token/csrf）。
  await loginAs(page, 'platform_admin', fx)

  // 在页面上下文 PATCH 实例语言为 en，复刻 api client 的鉴权头；断言 200 且返回 app.locale=en。
  const patched = await page.evaluate(async (appId) => {
    const token = localStorage.getItem('ocm.access_token')
    const csrf = document.cookie
      .split(';')
      .map((s) => s.trim())
      .find((s) => s.startsWith('csrf_token='))
      ?.split('=')[1]
    const res = await fetch(`/api/v1/apps/${appId}/locale`, {
      method: 'PATCH',
      headers: {
  ...
  expect(patched.body.app.locale).toBe('en')

  // 再 GET 实例详情二次确认 locale 持久为 en（GET 无需 CSRF，但仍需 Bearer）。
  const detail = await page.evaluate(async (appId) => {
    const token = localStorage.getItem('ocm.access_token')
  ...
  expect(detail.body.app.locale).toBe('en')
})
