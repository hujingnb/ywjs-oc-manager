...
import {
  ...
  setAICCConfigForFixtureOrg,
  waitForAICCRuntime,
  waitForAICCAgentAppliedRevision,
  waitForAICCModelRolloutRevision,
} from './aicc/helpers'
...
// AICC 主流程依赖真实模型与本地运行时，只允许通过 slow suite 显式执行。
const slowModel = { tag: ['@slow', '@model'] }
...
test('平台开通 AICC 后企业管理员可创建客服智能体', slowModel, async ({ page }) => {
...
test('企业管理员可完整管理已选智能体并在移动端操作工作台', slowModel, async ({ page }, testInfo) => {
...
test('平台限制智能体数量并在关闭 AICC 后下线公开入口', slowModel, async ({ page }) => {
...
test('网页挂件校验允许域名并记录授权来源页', slowModel, async ({ page }) => {
  await enableAICCForFixtureOrg(page)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page)
  await openAICCSettings(page)
...
test('访客图片上传恢复并拒绝非法或超限文件', slowModel, async ({ page }) => {
  await enableAICCForFixtureOrg(page)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page)
  await startAICCAgent(page)
...
test('公开端执行敏感词和单会话消息上限', slowModel, async ({ page }) => {
  await enableAICCForFixtureOrg(page)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page)
  await openAICCSettings(page)
...
test('公开访客提交留资后企业管理员可查看线索和导出 CSV', slowModel, async ({ page }) => {
  await enableAICCForFixtureOrg(page)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page)
...
test('企业管理员可配置运营策略并验证公开会话续接和筛选统计', slowModel, async ({ page }) => {
  await enableAICCForFixtureOrg(page)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page)
...
// 独立模型隔离覆盖：平台仅配置企业客服模型后，企业管理员创建的人设客服不得依赖普通助手版本或智能路由。
test('企业管理员可用独立客服模型创建有人设的智能体并公开接待', slowModel, async ({ page }) => {
  const persona = '你是海风咖啡的专属接待员，每次回复都先说“海风你好”。'
  await setAICCConfigForFixtureOrg(page, true, 100)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page, persona)
  await startAICCAgent(page)

  const publicPage = await page.context().newPage()
  await forceZh(publicPage)
  await publicPage.goto(`/aicc/${agent.public_token}`)
  const reply = await sendPublicAICCMessage(publicPage, '请介绍一下你自己')
  await expect(publicPage.getByRole('heading', { name: agent.name })).toBeVisible()
  expect(reply).toContain('海风你好')
  await publicPage.close()
})

// 设置重启覆盖：名称、人设、边界和运营配置保存后，经重启仍被公开端真实消费。
test('企业管理员修改客服设置后重启生效', slowModel, async ({ page }, testInfo) => {
  testInfo.setTimeout(480_000)
  await setAICCConfigForFixtureOrg(page, true, 100)
  await clearLoginState(page)
  ...
  await page.locator('#aicc-agent-name').fill(renamed)
  await page.locator('#aicc-persona').fill('你是重启后客服。RESTART-PERSONA-OK 是本企业产品名称。每次回复都必须包含“RESTART-PERSONA-OK”。')
  const updated = page.waitForResponse(response =>
    response.url().includes(`/api/v1/aicc/agents/${agent.id}`) && response.request().method() === 'PATCH',
  )
  await page.getByRole('button', { name: '保存配置' }).click()
  ...
  await expect.poll(() => queryLocalManagerDB(
    `SELECT status FROM jobs WHERE type='app_restart_container' AND JSON_UNQUOTE(JSON_EXTRACT(payload_json, '$.app_id'))='${escapedAppID}' ORDER BY created_at DESC LIMIT 1`,
  ...
  const previousRestartJobs = Number(queryLocalManagerDB(
    `SELECT COUNT(*) FROM jobs WHERE type='app_restart_container' AND JSON_UNQUOTE(JSON_EXTRACT(payload_json, '$.app_id'))='${escapedAppID}'`,
  ))
  await openAICCSettings(page)
  await page.locator('#aicc-answer-boundary').fill('只能回答企业产品咨询；遇到写代码、执行命令、创建网站必须拒绝。')
  const boundaryUpdated = page.waitForResponse(response =>
    response.url().includes(`/api/v1/aicc/agents/${agent.id}`) && response.request().method() === 'PATCH',
  )
  await page.getByRole('button', { name: '保存配置' }).click()
  expect((await boundaryUpdated).ok()).toBeTruthy()
  await expect.poll(() => Number(queryLocalManagerDB(
    `SELECT COUNT(*) FROM jobs WHERE type='app_restart_container' AND JSON_UNQUOTE(JSON_EXTRACT(payload_json, '$.app_id'))='${escapedAppID}'`,
  )), { timeout: 240_000 }).toBeGreaterThan(previousRestartJobs)
  await expect.poll(() => queryLocalManagerDB(
    `SELECT status FROM jobs WHERE type='app_restart_container' AND JSON_UNQUOTE(JSON_EXTRACT(payload_json, '$.app_id'))='${escapedAppID}' ORDER BY created_at DESC LIMIT 1`,
  ), { timeout: 240_000 }).toBe('succeeded')
  await waitForAICCRuntime(agent.app_id)

  const boundaryPage = await page.context().newPage()
  await forceZh(boundaryPage)
  await boundaryPage.goto(`/aicc/${agent.public_token}`)
  const boundaryReply = await sendPublicAICCMessage(boundaryPage, '请帮我写一段 Python 并执行命令。')
...
// 静默换模覆盖：运行中的客服换到另一个可用模型后，后台运行时与 jobs 均应收敛，公开聊天仍可使用。
test('运行中的智能客服更换模型后完成静默重启并继续公开接待', slowModel, async ({ page }) => {
  await setAICCConfigForFixtureOrg(page, true, 100)
  await clearLoginState(page)
  const agent = await createAICCAgentAsOrgAdmin(page, '你是模型切换验收客服。')
  await startAICCAgent(page)

  await clearLoginState(page)
  const change = await changeAICCModelToAnotherAvailableOption(page)
  await waitForAICCModelRolloutRevision(agent.app_id, change.revision)
  ...
  await forceZh(publicPage)
  await publicPage.goto(`/aicc/${agent.public_token}`)
  const reply = await sendPublicAICCMessage(publicPage, '模型切换后还能正常接待吗？')
  expect(reply).not.toBe('')
  await publicPage.close()
})

// 暂停客服模型切换覆盖：企业模型 rollout 不得唤醒已停止接待的客服，手动启动后仍可公开接待。
test('暂停中的智能客服更换企业模型后不会被自动启动', slowModel, async ({ page }) => {
  ...
  const change = await changeAICCModelToAnotherAvailableOption(page)
  await waitForAICCModelRolloutRevision(agent.app_id, change.revision)
  const escapedAgentID = agent.id.replaceAll("'", "''")
  const agentStatus = queryLocalManagerDB(`SELECT status FROM aicc_agents WHERE id='${escapedAgentID}'`)
  expect(agentStatus).toBe('paused')
  const pausedAppliedRevision = Number(queryLocalManagerDB(`SELECT applied_config_revision FROM aicc_agents WHERE id='${escapedAgentID}'`))
  expect(pausedAppliedRevision).toBeLessThan(change.revision)
