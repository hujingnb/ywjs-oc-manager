<template>
  <n-space :size="6">
    <n-button v-if="show('complete') && verbSupported('complete')" size="small" type="primary" @click="emit('action', 'complete')">标记完成</n-button>
    <n-button v-if="show('block') && verbSupported('block')" size="small" @click="emit('action', 'block')">阻塞</n-button>
    <n-button v-if="show('unblock') && verbSupported('unblock')" size="small" type="primary" @click="emit('action', 'unblock')">解除阻塞</n-button>
    <n-button v-if="show('reclaim') && verbSupported('reclaim')" size="small" @click="emit('action', 'reclaim')">释放 claim</n-button>
    <n-button v-if="show('reassign') && verbSupported('reassign')" size="small" @click="emit('action', 'reassign')">重新分配</n-button>
    <n-button v-if="show('comment') && verbSupported('comment')" size="small" @click="emit('action', 'comment')">评论</n-button>
    <n-button v-if="show('archive') && verbSupported('archive')" size="small" type="error" @click="emit('action', 'archive')">归档</n-button>
  </n-space>
</template>
