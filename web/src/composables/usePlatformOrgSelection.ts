// usePlatformOrgSelection 为平台管理员补齐企业选择上下文。
// 后端成员、应用、审计和知识库接口仍按 org_id 分域；平台管理员没有自身 org_id，
// 因此页面需要从企业列表中选出一个目标企业后再发起 org-scoped 查询。
import { computed, ref, watch, type Ref } from 'vue'

import { useOrganizationsQuery } from '@/api/hooks/useOrganizations'

// OrgSelectionUser 是企业选择逻辑需要的最小登录用户视图。
export interface OrgSelectionUser {
  // role 决定是否需要展示平台企业选择器。
  role?: string
  // org_id 是企业角色的默认企业；平台管理员通常为空。
  org_id?: string
}

// usePlatformOrgSelection 返回平台管理员可切换的企业 ID，以及企业角色的自身企业 ID。
export function usePlatformOrgSelection(
  ...
  // 路由显式传入 orgId 时优先使用路由上下文，避免覆盖企业详情页内的选择。
  watch(explicitOrgId, (orgId) => {
    if (orgId) selectedOrgId.value = orgId
  }, { immediate: true })

  // 平台管理员首次进入企业域页面时，默认选中第一个企业，避免列表因 org_id 为空而停留在空态。
