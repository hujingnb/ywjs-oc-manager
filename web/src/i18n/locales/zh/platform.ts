...
export default {
  ...
  orgs: {
    ...
    form: {
      ...
      labelAICCModel: '客服模型',
      labelAICCIndustryKnowledge: '授权行业知识库',
      ...
      placeholderAICCModel: '搜索并选择该企业的客服模型',
      placeholderAICCIndustryKnowledge: '选择该企业的 AICC 可用行业知识库（可多选）',
      placeholderVersions: '选择该企业可用的助手版本（可多选，可留空）',
      aiccModelRequired: '启用 AICC 时必须选择客服模型',
      aiccModelLoadFail: '客服模型目录加载失败，暂时无法保存',
      aiccConfigLoadFail: '企业 AICC 配置加载失败，暂时无法保存',
      aiccConfigNotReady: '企业 AICC 配置或客服模型目录尚未加载完成',
      aiccSelectedModelUnavailable: '当前选择的客服模型 {model} 已不在实时模型目录中，请重新选择',
    },
    columns: {
      name: '名称',
      code: '企业标识',
    ...
    aiccPartialSuccessError: '企业资料已保存，但 AICC 配置保存失败：{message}',
    modelChangeConfirm: {
      title: '确认更换客服模型',
      message: '更换模型后，系统将逐个静默重启该企业正在运行的智能客服，以应用新模型。是否继续？',
      confirmLabel: '确认更换',
    },
    rechargeModal: {
      title: '企业充值',
...
- 每个关联行业库都会在检索时单独召回最多 \`top_k\` 条结果，关联过多会增加上下文长度和响应成本。`,
  ...
  skills: {
    ...
    // versionInvalid：版本号格式非法时的内联提示，格式限定为 x.x.x 或 x.x（纯数字段）
    versionInvalid: '版本号必须为 x.x.x 或 x.x 格式',
    descLabel: '描述',
