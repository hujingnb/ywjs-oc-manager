export default {
  console: {
    eyebrow: 'AI Contact Center',
    title: 'AICC Console',
    returnToOverview: 'Back to overview',
    navLabel: 'AICC console content',
    checkingAccess: 'Checking customer-care access',
    currentAgent: 'Current agent',
    noAgentSelected: 'No agent selected',
    switchOrganization: 'Switch organization',
    switchAgent: 'Switch agent',
    createAgent: 'New agent',
    agentLoading: 'Loading agents',
    agentLoadFailed: 'Failed to load agents',
    knowledgeRedirect: {
      loading: 'Loading current agent knowledge base',
      noAgent: 'Select an agent first',
      loadFailed: 'Failed to load current agent knowledge base',
    },
    nav: {
      reception: 'Reception',
      sessions: 'Sessions',
      knowledge: 'Knowledge',
      leads: 'Leads',
      analytics: 'Analytics',
      settings: 'Settings',
    },
  },
  manager: {
    title: 'AICC Desk',
    subtitle: 'Manage customer-care agents, public links, and visitor privacy settings in one place.',
    createAgent: 'New agent',
    agents: 'Agents',
    loading: 'Loading',
    noScenario: 'No business scenario',
    emptyAgentsTitle: 'No customer-care agents yet',
    emptyAgentsDesc: 'Create one to bind a hidden instance and generate public entry points.',
    editAgent: 'Edit agent',
    newAgent: 'New agent',
    unnamedAgent: 'Unnamed agent',
    stopReception: 'Stop reception',
    startReception: 'Start reception',
    delete: 'Delete',
    tabs: {
      config: 'Agent config',
      sessions: 'Sessions',
      leads: 'Leads',
      analytics: 'Analytics',
    },
    sections: {
      config: {
        eyebrow: 'RECEPTION',
        title: 'Reception',
        description: 'Review the current agent status, public entry points, and widget delivery details.',
      },
      knowledge: {
        eyebrow: 'KNOWLEDGE',
        title: 'Knowledge',
        description: 'Manage the current agent knowledge base and configure industry and organization retrieval scope.',
      },
      sessions: {
        eyebrow: 'SESSIONS',
        title: 'Sessions',
        description: 'Review visitor sessions, message details, sources, and follow-up status.',
      },
      leads: {
        eyebrow: 'LEADS',
        title: 'Leads',
        description: 'Review visitor lead details, unread leads, and conversion status.',
      },
      analytics: {
        eyebrow: 'ANALYTICS',
        title: 'Analytics',
        description: 'Review session trends, active agents, and lead conversion.',
      },
      settings: {
        eyebrow: 'SETTINGS',
        title: 'Settings',
        description: 'Maintain agent profile, conversation rules, knowledge scope, and lead fields.',
      },
    },
    status: {
      runtime: 'Runtime status',
      retention: 'Retention',
      days: '{count} days',
      publicEntry: 'Public entry',
      generated: 'Generated',
      generatedAfterSave: 'Generated after save',
      active: 'Receiving',
      paused: 'Paused',
      deleted: 'Deleted',
      draft: 'Draft',
      starting: 'Starting',
      ready: 'Ready to receive',
      receiving: 'Receiving',
      error: 'Error',
      errorDetail: 'Runtime error details',
    },
    form: {
      name: 'Agent name',
      namePlaceholder: 'For example: Presales concierge',
      retentionDays: 'Data retention days',
      privacyMode: 'Privacy mode',
      greeting: 'Greeting',
      greetingPlaceholder: 'The first message visitors see when opening chat',
      scenario: 'Business scenario',
      scenarioPlaceholder: 'Describe the audience, question types, and handoff boundaries',
      persona: 'Customer-service persona',
      personaPlaceholder: 'Describe the agent identity, tone, service principles, and response style',
      model: 'Organization customer-service model',
      modelUnavailable: 'No model configured',
      configLoading: 'Loading the organization customer-service configuration. Please wait before saving.',
      configLoadFailed: 'Failed to load the organization customer-service configuration. Retry before saving.',
      industryKnowledge: 'Authorized industry knowledge bases',
      industryPlaceholder: 'Select industry knowledge bases this agent can retrieve',
      revokedIndustryKnowledge: 'Authorization revoked ({id})',
      answerBoundary: 'Answer boundary',
      answerBoundaryPlaceholder: 'For example: no pricing commitments, no refund approvals, suggest human follow-up for complaints',
      privacyText: 'Privacy notice',
      privacyTextPlaceholder: 'Explain what data is collected, how long it is stored, and how it is used',
      allowedDomains: 'Allowed widget domains',
      allowedDomainsPlaceholder: 'One domain per line, e.g. www.example.com or *.example.com. Leave blank to allow all.',
      reset: 'Reset',
      saveConfig: 'Save config',
      help: {
        name: 'The customer-care agent name shown on the public page and in the console. Use a role or service name visitors understand.',
        retentionDays: 'How many days visitor sessions, images, and leads are retained in this system before cleanup.',
        privacyMode: 'Controls whether visitors only see a privacy notice or must consent before starting a conversation.',
        greeting: 'The first message visitors see when opening the public page or widget.',
        scenario: 'Describes the audience, business questions, and service scenarios this agent should handle.',
        persona: 'Defines this agent’s identity, response style, and service principles; up to 8,000 characters.',
        model: 'The platform administrator centrally configures the model in the organization AICC configuration. It is read-only here.',
        industryKnowledge: 'Only organization-authorized industry knowledge bases are available; the selected scope saves with this agent.',
        answerBoundary: 'Defines what the agent must not promise or process, such as pricing commitments, refund approvals, or complaint escalation.',
        privacyText: 'The data-use notice shown to visitors, including collected data, purpose, and retention policy.',
        allowedDomains: 'Restricts which websites can load the widget. Leave blank to allow all domains.',
      },
    },
    delivery: {
      eyebrow: 'Delivery and safety',
      title: 'Public entry and session resume',
      publicTitle: 'Public entry',
      settingsTitle: 'Session resume and safety',
      blockedVisitors: 'Blocked visitors {count}',
      noAgent: 'Save the agent before configuring public entry and operational safety policies.',
      publicLink: 'Public link',
      copy: 'Copy',
      preview: 'Preview',
      qrAlt: 'AICC public-link QR code',
      qrPending: 'QR code appears after save',
      downloadPng: 'Download PNG',
      messageLimit: 'Messages per session',
      resumeTtl: 'Session resume TTL (minutes)',
      enableBlockedVisitors: 'Enable visitor blocking',
      sensitiveWords: 'Sensitive words (one per line)',
      sensitiveWordsPlaceholder: 'Matched content is blocked on the public page and visitors are asked to revise it',
      saveSettings: 'Save operations config',
      help: {
        messageLimit: 'Limits how many messages a single visitor session can send to prevent abusive or oversized conversations.',
        resumeTtl: 'The time window in which a visitor can refresh or reopen the link and resume the same conversation.',
        enableBlockedVisitors: 'When enabled, the system marks high-risk visitors based on sensitive-word hits or abnormal frequency and blocks them from sending more messages.',
        sensitiveWords: 'Words or phrases visitors are not allowed to send. Matched messages are blocked on the public page.',
      },
    },
    knowledge: {
      eyebrow: 'Knowledge scope',
      title: 'Agent retrieval sources',
      noAgent: 'Save the agent before configuring knowledge scope.',
      useOrgKnowledge: 'Use organization shared knowledge',
      industryKnowledge: 'Industry knowledge',
      industryPlaceholder: 'Select industry knowledge bases available to this agent',
      currentAgentKnowledge: 'Current agent knowledge base',
      currentAgentKnowledgeDesc: 'Stores materials used only by this customer-service agent. Upload and manage files on its knowledge base page.',
      manageCurrentKnowledge: 'Manage current agent knowledge',
      enabled: 'Enabled',
      save: 'Save knowledge scope',
      help: {
        useOrgKnowledge: 'Allows the agent to retrieve content from the organization shared knowledge base when answering.',
        industryKnowledge: 'Available industry knowledge bases are authorized by the platform administrator when creating or configuring the organization.',
        currentAgentKnowledge: 'The agent uses these materials when answering visitors. It is suitable for product notes, service scripts, and FAQs.',
      },
    },
    leadFields: {
      eyebrow: 'Visitor leads',
      title: 'Public-page contact fields',
      add: 'Add field',
      noAgent: 'Save the agent before configuring public lead fields.',
      empty: 'No lead fields configured. Visitors can start consulting directly.',
      labelPlaceholder: 'Field label',
      keyPlaceholder: 'Field key',
      required: 'Required',
      promptPlaceholder: 'Input hint',
      save: 'Save lead fields',
      defaultPhone: 'Phone',
      defaultField: 'Field {count}',
    },
    snippet: {
      placeholder: 'Embed snippet',
      previewWidget: 'Preview widget',
    },
    widgetPreview: {
      eyebrow: 'Widget Preview',
      title: 'Website widget preview',
      back: 'Back to reception',
      heroEyebrow: 'AICC customer service',
      heroTitle: 'Sample product consultation page',
      heroDescription: 'Use this page to preview how the widget appears on a company website. The online-service entry should appear at the bottom right.',
      cardsLabel: 'Preview content',
      cardPricingTitle: 'Pricing consultation',
      cardPricingText: 'Test package differences, trial options, activation flow, and applicable scenarios.',
      cardProductTitle: 'Product features',
      cardProductText: 'Test whether the agent answers feature and usage questions within the configured business scenario.',
      cardFaqTitle: 'Common questions',
      cardFaqText: 'Suggested tests: how to activate, whether org knowledge is supported, and how to embed the widget.',
    },
    deleteModal: {
      title: 'Delete AICC agent',
      message: 'The public link will stop working after deletion. Enter {name} to confirm.',
      confirm: 'Delete',
    },
    options: {
      privacyNotice: 'Show privacy notice',
      consentRequired: 'Require consent before reception',
      fieldText: 'Text',
      fieldPhone: 'Phone',
      fieldEmail: 'Email',
      fieldNumber: 'Number',
    },
    defaults: {
      greeting: 'Hello, I am the online assistant. How can I help?',
      privacyText: 'We use this conversation to answer your questions and store it according to the organization data retention policy.',
    },
    feedback: {
      missingLeadField: 'Complete lead field labels and keys',
      leadFieldsSaved: 'Lead fields saved',
      leadFieldsSaveFailed: 'Failed to save lead fields',
      knowledgeSaved: 'Knowledge scope saved',
      knowledgeSaveFailed: 'Failed to save knowledge scope',
      settingsSaved: 'Operations config saved',
      settingsSaveFailed: 'Failed to save operations config',
      missingName: 'Enter an agent name',
      configSaved: 'Config saved',
      saveFailed: 'Save failed',
      started: 'Reception started',
      stopped: 'Reception stopped',
      statusSwitchFailed: 'Failed to change status',
      deleted: 'Agent deleted',
      deleteFailed: 'Delete failed',
      copied: 'Copied',
      copyFailed: 'Copy failed. Select the text and copy it manually.',
    },
  },
  analytics: {
    rangeToday: 'Today',
    range7d: 'Last 7 days',
    range30d: 'Last 30 days',
    day: 'Day',
    week: 'Week',
    metrics: {
      todaySessions: 'Today sessions',
      filteredSessions: 'Filtered sessions',
      unreadLeads: 'Unread leads',
      resolved: 'Resolved',
      unresolved: 'Unresolved',
      unknown: 'Unknown',
      unresolvedRate: 'Unresolved rate',
      completedLeads: 'Completed leads',
      agentCount: 'Agents',
      activeAgents: 'Receiving',
    },
    trend: 'Session trend',
    noTrend: 'No trend data',
    leadStatus: 'Lead status',
    unread: 'Unread',
    regions: 'Visitor regions',
    noRegions: 'No region data',
    questions: 'Top questions',
    noQuestions: 'No question data',
    sources: 'Source pages',
    noSources: 'No source data',
  },
  sessions: {
    selectAgentTitle: 'Select an agent to view sessions',
    selectAgentDesc: 'Sessions are sorted by recent creation time for quick operations review.',
    recentSessions: 'Recent sessions',
    filters: {
      resolution: 'Resolution',
      lead: 'Lead status',
      channel: 'Channel',
      region: 'Region',
      startTime: 'Start time',
      endTime: 'End time',
      keyword: 'Source keyword',
    },
    emptyTitle: 'No visitor sessions',
    emptyDesc: 'Conversations from public links appear here.',
    unknownRegion: 'Unknown region',
    resolved: 'Resolved',
    unresolved: 'Unresolved',
    followUp: 'Following up',
    messageCount: '{count} messages',
    unselected: 'No session selected',
    noMessagesTitle: 'No messages',
    noMessagesDesc: 'Select a session on the left to view visitor and assistant messages.',
    summary: {
      region: 'Region',
      messageCount: 'Messages',
      channel: 'Channel',
      source: 'Source',
    },
    messageFallbackImage: 'Image message',
    messageFallbackEmpty: 'Empty message',
    imageTag: 'Image',
    fallbackTag: 'Fallback',
    refusalTag: 'Refusal',
    resolutionOptions: {
      resolved: 'Resolved',
      unresolved: 'Unresolved',
      unknown: 'Following up',
    },
    leadOptions: {
      pending: 'Pending',
      complete: 'Completed',
      skipped: 'Skipped',
    },
    channelOptions: {
      webLink: 'Public link',
      webWidget: 'Web widget',
    },
    roles: {
      assistant: 'Assistant',
      system: 'System',
      visitor: 'Visitor',
    },
  },
  leads: {
    title: 'Visitor leads',
    exportCsv: 'Export CSV',
    emptyTitle: 'No visitor leads',
    emptyDesc: 'Visitor information recognized from sessions will appear here.',
    unnamedVisitor: 'Unnamed visitor',
    anonymousIntentLead: 'Anonymous interested visitor',
    sourceLabel: 'Source',
    unconfirmedNetwork: 'Public web, not confirmed by the organization',
    sessionPrefix: 'Session {id}',
    unread: 'Unread',
    read: 'Read',
    viewConversation: 'View chat',
    conversationTitle: "{name}'s conversation",
    conversationDialogLabel: 'Lead conversation',
    closeConversation: 'Close conversation',
    noConversationTitle: 'No conversation yet',
    noConversationDesc: 'This lead has no session messages available yet.',
    messageFallbackImage: 'Image message',
    messageFallbackEmpty: 'Empty message',
    imageTag: 'Image',
    fallbackTag: 'Fallback reply',
    refusalTag: 'Refusal',
    markRead: 'Mark read',
    markedRead: 'Marked as read',
    markFailed: 'Failed to mark as read',
    exportFailed: 'Export failed',
    intentProfileLabel: 'Intent profile',
    intentProfileTitle: 'Intent profile',
    intentNoFields: 'No verifiable business fields yet',
    intentLevels: { low: 'Low intent', medium: 'Medium intent', high: 'High intent', unknown: 'Pending analysis' },
  },
  publicChat: {
    defaultTitle: 'Online support',
    online: 'Online',
    connecting: 'Connecting',
    ready: 'Ready',
    newConversation: 'New chat',
    endConversation: 'End this consultation',
    conversationEnded: 'This consultation has ended. Reopen the support entry to start again.',
    resolved: 'Resolved',
    unresolved: 'Unresolved',
    messageListLabel: 'Customer-care messages',
    uploadedImageAlt: 'Visitor uploaded image',
    typing: 'Replying...',
    queued: 'Your message is queued for processing.',
    busy: 'The assistant is working on your message.',
    retrying: 'The service is busy and will retry shortly.',
    failed: 'The reply failed. Please retry.',
    failedNoRetry: 'The reply failed. This historical message cannot be retried; please send your question again.',
    retry: 'Retry',
    consentTitle: 'Confirm privacy notice before continuing',
    consentButton: 'Agree and start',
    leadTitle: 'Please leave contact information first',
    submitLead: 'Submit contact info',
    declineLead: 'Continue without sharing details',
    resolutionTitle: 'Did this answer your question?',
    continueChat: 'Continue chatting',
    sourceLabel: 'Source',
    unconfirmedNetwork: 'Public web, not confirmed by the organization',
    chooseImage: 'Choose image',
    pendingImageAlt: 'Pending image',
    removeImage: 'Remove image',
    composerPlaceholder: 'Enter your question',
    send: 'Send',
    defaultPrivacyText: 'We use this conversation to answer your questions.',
    defaultGreeting: 'Hello, I am the online assistant. How can I help?',
    missingField: 'Please fill in {label}',
    missingRequired: 'Complete the required contact information',
    leadRequired: 'Submit contact information before continuing.',
    sensitiveWord: 'This message contains unsupported content. Please revise and try again.',
    messageLimit: 'This session has reached the message limit. Please reopen support later.',
    visitorBlocked: 'This visitor cannot continue consulting right now.',
    sendFailed: 'Failed to send message. Please try again later.',
    imageTooLarge: 'Image must be 10MiB or smaller',
    imageTypeInvalid: 'Please choose an image file',
  },
}
