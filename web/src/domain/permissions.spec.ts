...
// 重点验证 org_admin 在 orgId 省略和显式传入两种场景下均可管理自身企业知识库。
...
describe('canManageOrgKnowledge', () => {
  ...
  // 覆盖 ORG_ADMIN 不传 orgId（页面未显式传入企业上下文）时仍可管理自身企业知识库。
  ...
  // 覆盖 ORG_ADMIN 传入跨企业 orgId 时无权管理（企业边界隔离）。
  ...
  // 覆盖 platform_admin 跨企业也可管理企业知识库
  ...
  // 覆盖 org_member 无法管理企业知识库（仅 org_admin 可写）。
