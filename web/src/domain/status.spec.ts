...
// 1) appStatusViews 对 5 个新 init 子状态 + 重命名后的 draft 文案是否对齐；
...
describe('formatAppStatus', () => {
  ...
  it.each([
    ...
    ['pulling_image', '拉取运行时镜像', 'warning'], // 拉取阶段：可能耗时，warning 提示进行中。
    ['syncing_image', '同步镜像到节点', 'warning'], // 同步阶段：多节点分发镜像，可能阻塞。
...
describe('isInitPhase', () => {
  // 5 个 init 子状态都返回 true：AppOverviewTab 据此渲染进度条。
