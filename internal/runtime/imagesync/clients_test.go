...
func TestAgentHTTPClientInspectImage(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/images/inspect" || r.URL.Query().Get("image") != "hermes-runtime:dev" {
	...
	info, err := AgentHTTPClient{BaseURL: server.URL, Token: "token"}.InspectImage(context.Background(), "node-1", "hermes-runtime:dev")
...
func TestAgentHTTPClientInspectImageUsesConfiguredTLSClient(t *testing.T) {
	...
	_, err := AgentHTTPClient{BaseURL: server.URL}.InspectImage(context.Background(), "node-1", "hermes-runtime:dev")
	require.Error(t, err)

	pool := x509.NewCertPool()
	pool.AddCert(server.Certificate())
	client := &http.Client{Transport: &http.Transport{TLSClientConfig: &tls.Config{RootCAs: pool}}}
	info, err := AgentHTTPClient{BaseURL: server.URL, HTTPClient: client}.InspectImage(context.Background(), "node-1", "hermes-runtime:dev")
	require.NoError(t, err)
