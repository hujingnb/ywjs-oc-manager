...
// channel_type=feishu / domain（feishu|lark）。
func feishuStartJob(t *testing.T, feishuDomain string) sqlc.Job {
	t.Helper()
	payload := `{"app_id":"` + testChannelWorkerAppID + `","channel_type":"feishu","domain":"` + feishuDomain + `"}`
