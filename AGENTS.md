...
> 仅本地 k3d 联调环境使用，不得用于任何线上 / 共享环境，也不要复用到生产。

| 服务 | 本地地址 | 账号 | 密码 |
|---|---|---|---|
| manager 后台 | http://ocm.localhost | `admin`（组织标识留空，即平台管理员）| `admin123` |
| manager 后台 | http://ocm.localhost | 企业标识 `demo-full` / 账号 `admin` | `admin123` |
| manager 后台 | http://ocm.localhost | 企业标识 `demo-full` / 账号 `member` | `member123` |
| manager 后台 | http://ocm.localhost | 企业标识 `demo-app` / 账号 `admin` | `admin123` |
| manager 后台 | http://ocm.localhost | 企业标识 `demo-app` / 账号 `member` | `member123` |
| manager 后台 | http://ocm.localhost | 企业标识 `demo-aicc` / 账号 `admin` | `admin123` |
| manager 后台 | http://ocm.localhost | 企业标识 `demo-aicc` / 账号 `member` | `member123` |
...
- 演示企业密码是 clean `make local-reset` 首次创建时的默认值；复用已有 `.k3d-data`
  执行 `make local-up` 只补缺，不会重置已修改的密码。
...
- 修复阶段必须优先运行覆盖改动点的最小相关测试（包括对应单元、集成或浏览器场景），不得因单个局部修复反复执行全量回归；只有修改会影响多个场景的共享基础设施、状态管理或运行时调度时，才可提前运行全量回归。
- 全量 E2E 不作为日常开发、局部修复或正常上线的默认验证；浏览器验证始终仅运行覆盖改动点的定向 spec。只有用户明确要求跨模块全量校准时，才执行全量 E2E。
- 定向 E2E 与浏览器验证默认使用无头（headless）模式执行；只有用户明确要求观察交互过程或进行可视化调试时，才启用有头模式。
...
- 所有新功能开发完成后，必须使用真实浏览器（而非 curl 或 API 调用）进行全面功能验证；curl 无法验证前端逻辑，不能替代浏览器验证。若发现问题，必须先修复并重新验证，直到功能正常后再交付。
- 所有定向修复验证通过后，交付前执行与改动范围匹配的定向回归；只有用户明确要求或共享基础设施改动确有跨模块风险时，才追加全量回归。全量回归发现失败时，应回到最小相关测试定位和修复，避免每次局部修改后立即重复全量执行。
- 确认文档、注释和命名与实际行为一致。
...
- If two functions differ only in one parameter value, merge them into one function with that parameter. Don't create separate wrapper functions.
