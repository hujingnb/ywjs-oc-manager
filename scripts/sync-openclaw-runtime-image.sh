...
if grep -Fq "\"id\":\"${local_id}\"" <<<"$remote_payload"; then
  echo "runtime node 已存在匹配 OpenClaw 镜像，跳过传输：${image} ${local_id}"
  exit 0
fi
...
if ! grep -Fq "\"id\":\"${local_id}\"" <<<"$loaded_payload"; then
  echo "OpenClaw 镜像下发后 hash 仍不一致：local=${local_id}, remote=${loaded_payload}" >&2
  exit 1
fi

echo "OpenClaw 镜像已同步到 runtime node：${image} ${local_id}"
