...
每次使用本 skill 完成一个 Hermes 版本新增/升级后，都要做一次简短复盘：

- 实际是否遇到本 skill 未覆盖的上游结构变化、构建失败、验证缺口或文件同步遗漏。
- 如果遇到，立即把可复用经验补回本 skill；只记录未来 agent 需要执行的规则，不记录一次性流水账。
- 如果流程中某一步可以更早暴露问题（例如新增离线模拟、守卫测试或缓存清理），优先把它固化为明确步骤。
- 如果新增内容只适用于 AICC、生产集群操作或其它独立领域，不塞进本 skill；改用对应 skill 或新建更小的 skill。
- skill 更新应和版本实现提交分开提交，提交信息使用 `docs(skill): ...`。

交付前明确说明本次是否更新了 skill；若没有更新，说明没有发现可复用的新规则。
