...
- **写操作 / 变更类**（`apply`/`create`/`replace`/`patch`/`edit`/`delete`/`rollout restart`/
  `scale`/`set image`，以及 `make prod-deploy-*`、`make prod-update-config` 这类内部封装了写的命令）：
...
| `make prod-deploy-hermes` / `prod-deploy-ops` | 构建推送 + 把新镜像 ref 写回 secret.yaml + `prod-update-config` |
| `make prod-update-config` | apply secret.yaml + 重启 manager-api 使新配置生效 |

注意 `prod-update-config` 重启的是 **manager-api**。如果你改的是 secret 里 **其他服务**的配置
（如 ragflow 的 ES/Redis、new-api 连接串），要 apply secret 后**单独重启那个服务**，
而不是 `prod-update-config`：
