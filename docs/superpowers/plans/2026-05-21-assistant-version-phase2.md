...
**Goal:** 让 Hermes 镜像的 oc-entrypoint 支持 manifest 契约 v2——智能路由（8 个 auxiliary 槽位）、版本 skill tar、仅平台层 prompt——并新建第二个 `hermes-v2026.5.15` variant 用于测试版本切换。
...
- 工作目录：所有改动在 `runtime/hermes/hermes-v2026.5.16/`；最后一个任务把它整体拷贝成 `hermes-v2026.5.15/`。
...
| `runtime/hermes/hermes-v2026.5.16/migrator/from_hermes_v2026_5_15.py` | 5.15→5.16 no-op 迁移 | 新建 |
| `runtime/hermes/hermes-v2026.5.15/**` | 第二个 variant（5.16 的完整拷贝） | 新建 |
...
## Task 6：新建 hermes-v2026.5.15 variant

**Files:**
- Create: `runtime/hermes/hermes-v2026.5.16/migrator/from_hermes_v2026_5_15.py`
- Create: `runtime/hermes/hermes-v2026.5.15/**`（整目录）

第二个 variant 是 `hermes-v2026.5.16` 完成 Phase 2 改造后的**完整拷贝**，仅 `version.txt` 与目录名不同，并互补一个 no-op 迁移模块，使两个 variant 之间可互相切换。

- [ ] **Step 1：给 5.16 variant 加 5.15→5.16 的 no-op 迁移**

创建 `runtime/hermes/hermes-v2026.5.16/migrator/from_hermes_v2026_5_15.py`：

```python
"""hermes-v2026.5.15 → hermes-v2026.5.16 的迁移。

两个 variant 渲染逻辑完全一致（5.15 是 5.16 的拷贝），数据无需迁移，no-op。
"""
...
def run(data_root: Path) -> dict:
    ...
    return {"from": "hermes-v2026.5.15", "to": "hermes-v2026.5.16", "mode": "noop"}
```
...
```bash
cp -r runtime/hermes/hermes-v2026.5.16 runtime/hermes/hermes-v2026.5.15
```

然后清理拷贝产生的非源码物：删除 `runtime/hermes/hermes-v2026.5.15/` 下的 `__pycache__`、`.pytest_cache`、注入的 `kanban-contract` / `cron-contract`（这些是构建期注入物，被 `.gitignore` 排除；用 `make hermes-inject-contract` 在构建时再生）。执行：

```bash
find runtime/hermes/hermes-v2026.5.15 -name __pycache__ -type d -exec rm -rf {} + 2>/dev/null; true
rm -rf runtime/hermes/hermes-v2026.5.15/.pytest_cache runtime/hermes/hermes-v2026.5.15/v
rm -rf runtime/hermes/hermes-v2026.5.15/kanban-contract runtime/hermes/hermes-v2026.5.15/cron-contract
```

（对照 `runtime/hermes/hermes-v2026.5.16/.gitignore` 确认哪些是忽略物——只提交源码文件。）

- [ ] **Step 3：改 5.15 variant 的 `version.txt`**

把 `runtime/hermes/hermes-v2026.5.15/version.txt` 内容从 `v2026.5.16` 改为 `v2026.5.15`（仅此一行，无换行差异）。

- [ ] **Step 4：调整 5.15 variant 的 migrator**

`runtime/hermes/hermes-v2026.5.15/migrator/__init__.py` 是从 5.16 拷来的，其中 legacy no-op 判断硬编码了 `curr_variant == "hermes-v2026.5.16"`。把该行改为 `curr_variant == "hermes-v2026.5.15"`，使 `hermes-main → hermes-v2026.5.15` 仍是 no-op。

删除拷贝来的 `runtime/hermes/hermes-v2026.5.15/migrator/from_hermes_v2026_5_15.py`（5.15 自己不需要「从 5.15 迁移」），改为创建 `runtime/hermes/hermes-v2026.5.15/migrator/from_hermes_v2026_5_16.py`：
...
- [ ] **Step 5：更新 5.15 variant 的 CONTRACT.md**

把 `runtime/hermes/hermes-v2026.5.15/CONTRACT.md` 里的 variant 名 `hermes-v2026.5.16` 改为 `hermes-v2026.5.15`，并把标题改为 `# hermes-v2026.5.15 · Variant 契约`。其余内容保持。

- [ ] **Step 6：自检 5.15 variant**

Run（在 `runtime/hermes/hermes-v2026.5.15/`）：`python3 -m pytest tests/ -v` — Expected: 全部 PASS（5.15 是 5.16 的拷贝，测试同样通过）。

- [ ] **Step 7：校验 Makefile variant 守卫**

Makefile 的 `.guard-hermes-version` 要求目录名 = `hermes-<version.txt 内容>`。确认 `runtime/hermes/hermes-v2026.5.15/version.txt` 是 `v2026.5.15`、目录名是 `hermes-v2026.5.15`、二者对齐。若本地有 Docker，可选：`make build-hermes-runtime HERMES_VARIANT=hermes-v2026.5.15` 验证构建（构建末尾跑 pytest）。无 Docker 跳过并注明。

- [ ] **Step 8：提交**

```bash
git add runtime/hermes/hermes-v2026.5.16/migrator/from_hermes_v2026_5_15.py runtime/hermes/hermes-v2026.5.15/
git commit -m "feat(hermes-runtime): 新增 hermes-v2026.5.15 variant 与互通迁移"
```
...
- §4.1 第二个 variant `hermes-v2026.5.15` → Task 6。
