...
**Goal:** 对当前 AICC 分支建立无遗漏、可重复的本地严格上线门禁，修复发现的问题，并给出有证据支持的 GO / NO-GO 结论。

**Architecture:** 先用需求矩阵与 `master...HEAD` 代码反查建立双向覆盖，再将验证分为自动化回归、真实浏览器、安全隔离、故障恢复、容量稳定性和升级回滚六层。业务缺陷在现有 AICC 模块内按 TDD 最小修复；每层独立提交，并在最终提交、最终镜像和干净数据上全量复跑。
...
- 修改 `web/tests/e2e/aicc.spec.ts`：保留 AICC 核心管理、公开会话、线索、统计和移动端闭环。
- 修改 `web/tests/e2e/aicc-access-i18n.spec.ts`：补齐平台管理员、企业管理员、普通成员、跨企业和中英文场景。
- 修改 `web/tests/e2e/aicc-knowledge.spec.ts`：补齐三类知识库上传、检索、取消范围和失败路径。
- 创建 `web/tests/e2e/aicc-public-chat.spec.ts`：承载挂件、图片、域名和留资边界，避免继续扩大核心流程文件。
...
### Task 0: 冻结基线并反查全部 AICC 变更

**Files:**
- Modify: `docs/testing/aicc-requirement-matrix.md`
- Create: `docs/testing/aicc-change-coverage.md`

- [ ] **Step 1: 记录最终验证基线并使历史结论失效**

在矩阵头部写入 `git rev-parse HEAD`，把所有历史 `PASS` 重置为 `BLOCKED`。保留历史证据文字，但加上“待最终基线复跑”前缀，避免把不同提交上的结果拼成最终结论。

Run:

```bash
git rev-parse HEAD
rg -n '\| PASS \|' docs/testing/aicc-requirement-matrix.md
```

Expected: 第一条命令输出 40 位 SHA；第二条命令在重置后无输出。

- [ ] **Step 2: 导出当前分支相对 master 的实现入口**

Run:

```bash
git diff --name-only master...HEAD | rg '(^|/)(aicc|AICC)|public_aicc|organization|authorizer|industry_knowledge|oc-kb|render_skills|seed-e2e|openapi|deploy/k8s' | sort -u
rg -n 'aicc|AICC' internal/api/router.go internal/api/handlers internal/service internal/auth internal/store/queries internal/migrations web/src/app web/src/api web/src/pages web/src/layouts web/public runtime/hermes/hermes-v2026.7.1 cmd/server deploy/k8s/local
```

Expected: 得到路由、API、service、权限、SQL、迁移、定时任务、前端、挂件、runtime 和部署配置的完整候选清单。

- [ ] **Step 3: 建立代码到需求的反向覆盖表**

在 `docs/testing/aicc-change-coverage.md` 逐文件记录职责和矩阵 ID：

```markdown
| 实现入口 | 职责 | 需求矩阵 ID | 自动化用例 | 浏览器/环境场景 | 状态 |
|---|---|---|---|---|---|
| `internal/api/handlers/public_aicc.go` | 匿名访客会话、消息、留资与状态接口 | AICC-SESSION-01..07、AICC-LEAD-01..03、AICC-STATUS-01..02 | 待最终复跑 | 待最终复跑 | BLOCKED |
```

每个候选文件必须至少关联一个矩阵 ID。一个文件包含多个独立行为时拆成多行；没有对应需求的行为先补充矩阵 ID，再填写自动化和浏览器场景。
...
```bash
rg -n '\| *(待映射|无|N/A) *\|' docs/testing/aicc-change-coverage.md
rg -n '\| *(待执行|待最终复跑) *\|.*\| PASS \|' docs/testing/aicc-requirement-matrix.md docs/testing/aicc-change-coverage.md
```

Expected: 两条命令均无输出；`N/A` 只能用于需求明确排除的语音和外部即时通信渠道，并在同一行写明规格依据。

- [ ] **Step 5: 提交验证基线与反查表**

```bash
git add docs/testing/aicc-requirement-matrix.md docs/testing/aicc-change-coverage.md
git commit -m "test(aicc): 冻结严格验证覆盖基线" -m "重置历史通过状态，并将当前分支全部客服实现入口反向映射到需求、自动化和浏览器场景。"
```
...
```bash
npm --prefix web run test:e2e -- aicc-access-i18n.spec.ts
...
```bash
npm --prefix web run test:e2e -- aicc.spec.ts aicc-public-chat.spec.ts
```
...
make openapi-check
npm --prefix web run test:e2e -- aicc.spec.ts aicc-access-i18n.spec.ts aicc-knowledge.spec.ts aicc-public-chat.spec.ts
...
git diff --check
git status --short
rg -n "FAIL|BLOCKED|未完成|待补充|待最终复跑|token|password" docs/testing/aicc-*.md
```
