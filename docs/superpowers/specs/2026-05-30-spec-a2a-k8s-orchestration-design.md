...
- **渠道绑定后的 hermes 重载（platform reload）k8s 化**：微信扫码绑定成功后需重启
  hermes 让其重新扫描 `/opt/data/weixin/accounts/` 加载新 platform。A2a 仅迁移了
  绑定身份解析（DockerBindingResolver→OcOpsBindingResolver），重启仍走 docker
  `ChannelRestarter`（k8s 下 nodeID/containerID 恒空，调用失败被日志吞、不阻断绑定
  状态闭环）。A2b 改为 orch 驱动的 pod 重启（Scale(0)→Scale(1) 或删 pod 触发
  Deployment 重建）。**影响**：A2a 渠道绑定状态闭环（→running）成立，但绑定后到
  下次 pod 重建之间，新 platform 可能未被 hermes 加载——属 A2b 完整渠道闭环验证范围。
