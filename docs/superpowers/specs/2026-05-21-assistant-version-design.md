...
`file_path` 相对 manager 数据根目录；tar 字节存文件系统主副本。`name` 从 tar 内 `SKILL.md` 的 YAML frontmatter `name` 字段推导，不由上传方手填。
...
hermes:
  runtime_images:
    ...
    - id: "v2026.5.7"
      ...
      ref: "crpi-nu3ibz4f07feyghi.cn-beijing.personal.cr.aliyuncs.com/ywjs_app/oc-manager-hermes:v2026.5.7-..."
```
...
新建 `runtime/hermes/hermes-v2026.5.7/`：完整拷贝当前 `hermes-v2026.5.16/`，**仅** `version.txt` 改为 `v2026.5.7` 且目录名与之对齐（Makefile `.guard-hermes-version` 要求），oc-entrypoint 渲染逻辑不做任何差异。用于测试版本切换与跨 variant 数据迁移（`migrator/`）。
...
针对 `runtime/hermes/hermes-v2026.5.16/`（以及新建的 `hermes-v2026.5.7/`）：
...
- skill tar 上传 / 删除：multipart 上传，校验合法 tar、大小上限 10 MiB、tar 内必须含 `SKILL.md`；skill 名称从 tar 内 `SKILL.md` 的 YAML frontmatter `name` 字段推导（同一版本内 `name` 重复则拒绝上传）；写 manager 文件系统主副本，更新 `skills_json`。
...
- 端到端：用 v2026.5.16 与 v2026.5.7 两个版本走完整版本切换 + 重启更新流程，真实浏览器验证。
...
2. 配置镜像列表 + 镜像列表只读 API + manifest 契约 v2 + oc-entrypoint 全部改动 + 新建 `hermes-v2026.5.7` variant。
