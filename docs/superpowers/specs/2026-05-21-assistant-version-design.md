...
新建 `runtime/hermes/hermes-v2026.5.15/`：完整拷贝当前 `hermes-v2026.5.16/`，**仅** `version.txt` 改为 `v2026.5.15` 且目录名与之对齐（Makefile `.guard-hermes-version` 要求），oc-entrypoint 渲染逻辑不做任何差异。用于测试版本切换与跨 variant 数据迁移（`migrator/`）。
