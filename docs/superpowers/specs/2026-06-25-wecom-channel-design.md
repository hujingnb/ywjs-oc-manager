...
> 的 git 历史）。
>
> **企业微信仍走 `internal/integrations/channel` 的 adapter 抽象**（与微信/飞书统一）：worker
> 的 `channel_check_binding`/`channel_start_login` 在 `load()` 里都会 `registry.Lookup(channelType)`
> （`channel_login.go:174,624`），未注册 adapter 的渠道无法进入 worker 流水线。因此企业微信
> 注册一个 `WorkWeChatAdapter`，由它的 `PollAuth` 承载「连通状态检查」（查 oc-ops health），
> 插进 worker 现成的**通用 check 路径**（`channel_login.go:270-338`，飞书才走特判分支）。
> 企业微信不需要的只是「扫码 SSE 取凭证」那段——凭证随 HTTP body 同步到达，由 service 直接
> 落库 + 注入，无需 adapter 的 SSE 消费 / `TakeCredentials`。
...
| `internal/integrations/channel` adapter | `WeChatAdapter`（SSE） | `FeishuAdapter`（SSE 消费 + TakeCredentials） | **`WorkWeChatAdapter`（无 SSE；`PollAuth` 查 oc-ops health）** |
| worker check 路径 | 通用路径（adapter.PollAuth 判 bound） | 飞书特判两阶段（注入→health） | **通用路径（复用 wechat 那条，adapter.PollAuth=health）** |
...
一句话：企业微信 = **注册 `WorkWeChatAdapter` 走统一 adapter 抽象 + 取凭证纯手填（无扫码
SSE）+ 注入运行复用飞书 env 链路 + 连通检查走 worker 通用 check 路径**。
...
## 5. 绑定流程（端到端，纯手填 + 同步注入 + 通用 check）

设计要点：凭证随 HTTP body 同步到达，故**注入在 service 发起时同步完成**（与现有 `Unbind`
同步 patch+restart 先例一致），worker 只负责「连通状态检查」走通用 check 路径——比飞书的
两阶段 SSE 特判简单得多。
...
2. **新增 `ChannelService.BeginWorkWechatAuth`**（`internal/service/channel_service.go`，仿 `BeginFeishuAuth:216` 但**省去 SSE/扫码**，改为同步注入）：
   - 权限校验 `CanManageApp`（`internal/auth/authorizer.go`，不在 service 内联角色判断）。
   - 实例状态守卫 `AppCanInitiateChannelAuth`（拦截 `restarting` 等）。
   - `registry.Lookup(work_wechat)` 渠道可用性守卫（与 `BeginFeishuAuth:229` 同口径，要求 adapter 已注册）。
   ...
   - **同步注入**：`PatchSecretKeys` 给 `app-<id>-token` Secret 写 `wecom-bot-id`/`wecom-secret` → `RolloutRestart` → `SetAppStatus(restarting)`（这套 patch+restart 依赖飞书 `SetFeishuUnbindDeps` 已注入的 `FeishuSecretPatcher`+`ChannelRestarter`，企业微信复用同一对依赖，必要时改名为渠道通用）。
...
3. worker `ChannelCheckBindingHandler`（`channel_login.go:250`）：`work_wechat` **走通用（非飞书）分支**（行 270-338）——`adapter.PollAuth` 即查 oc-ops health：
   - **`WorkWeChatAdapter.PollAuth`** 经 endpoint resolver 解析 app→oc-ops 坐标，调 `ChannelStatus(work_wechat)` 读 `platforms.wecom.platform_state`，映射 `connected`→`AuthStatusBound`、`fatal`→`AuthStatusFailed`(带 error)、其余→`AuthStatusPending`（结构仿 `ocOpsFeishuHealthClient:523`，但收口到 adapter 内，使通用 check 路径直接可用）。
   - 通用分支据此：`Bound`→`finalizeChannelBound`（写 bound + `binding_waiting`→`running`<sup>注</sup> + 审计 `channel_bound:succeeded`）；`Failed`→置 failed 带原因 + 审计；`Pending`→按退避 re-enqueue 继续等连接（注入时已重启，pod 起来后引擎自连）。
   - <sup>注</sup>`finalizeChannelBound` 现仅对 wechat 触发重启（`channel_login.go:577`）——企业微信**无需在此重启**（绑定发起时已 `RolloutRestart` 载入凭证），保持现状即可。
4. 前端轮询进度（`GET /api/v1/apps/:appId/channels/:channelType/auth` 的 `PollAuth`，service 侧直接读 DB binding，渠道无关）展示「验证中 → 已连接 / 失败原因」；`restarting` 窗口由 `instanceReady` 闸门提示。

### 路由

```
POST   /api/v1/apps/:appId/channels/work_wechat/auth     → BeginWorkWechatAuth（手填提交配置 + 同步注入）
GET    /api/v1/apps/:appId/channels/work_wechat/auth     → PollAuth（连通态进度，读 DB）
...
- manager 侧 oc-ops 客户端（`internal/integrations/ocops/`）+ service 接口（`internal/service/ocops.go` 的 `channelOps`）：`ChannelStatus(ctx, ep, "work_wechat")` 复用飞书已有的 `ChannelStatus` 方法（已按 channel 参数化，零新增）。
- **`WorkWeChatAdapter` 持有 endpoint resolver + oc-ops `ChannelStatus` 客户端**（构造期注入，结构仿 `ocOpsFeishuHealthClient:523`），其 `PollAuth` 即「解析坐标 → 查 health → 映射 AuthStatus」。这样「连通状态检查」收口在 adapter 内，worker 通用 check 路径无需 wecom 特判。`BeginAuth` 对企业微信不参与流程（无 `channel_start_login`），实现为返回错误/空挑战的占位即可；`Type()` 返回 `work_wechat` 供 registry 路由。
...
- 不做扫码 SSE 取凭证 / adapter 的 `TakeCredentials`（凭证随 HTTP body 同步到达，service 直接落库注入）；但**仍注册 `WorkWeChatAdapter`** 走统一抽象（见第 1、5、6 节）。
- 不做多机器人 / 多账号。
...
| adapter | `internal/integrations/channel/work_wechat.go`（新） | `WorkWeChatAdapter`：`Type/BeginAuth(占位)/PollAuth(查 oc-ops health)`；持 resolver+ChannelStatus 客户端；`cmd/server/main.go` 注册进 Registry（仿飞书） |
...
| service | `internal/service/channel_service.go:216,368-393` | 新增 `BeginWorkWechatAuth`（仿 `BeginFeishuAuth` 去 SSE，改同步 patch+restart）；解绑复用飞书链路 |
| worker | `internal/worker/handlers/channel_login.go:270-338` | **复用通用 check 分支**（adapter.PollAuth 判 bound），不加 work_wechat 特判；不入 `channel_start_login` |
