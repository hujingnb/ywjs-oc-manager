...
| worker | 1–5，HPA CPU/内存 | 执行抓取任务、消费队列 | 无状态 |
...
API、worker、Playwright 是可横向扩容的计算组件；Redis、RabbitMQ、PostgreSQL 是单副本临时依赖，
不作伪高可用声明。所有 Deployment 使用 `Recreate` 或安全的单副本策略，数据库不挂 PVC、不接 S3。
任一基础依赖重启会丢失排队与执行中的任务；Hermes 将该次 `web_extract` 视为失败并正常告知信息
无法读取，不得编造正文或来源。

初始资源建议：API 250m/512Mi（上限 1CPU/1Gi），worker 500m/1Gi（上限 2CPU/2Gi），
Playwright 500m/1Gi（上限 2CPU/2Gi）；实际并发由 Playwright 的 CPU/内存阈值和 HPA 收敛，初期
以 2–5 个动态页面并发为容量目标，再根据指标调整。
...
所有受支持 Hermes runtime variant 的 `render_config_yaml.py` 写入一致配置，并由 Deployment 为 Hermes
容器注入集群内服务地址：
...
同时注入 `FIRECRAWL_API_URL=http://firecrawl-api.oc-firecrawl.svc.cluster.local:3002`。具体配置键以固定的
Hermes 上游版本实际 schema 为准，并由 renderer 测试断言。AICC 的 `web` toolset 保持可用；普通
Hermes 需显式启用其现有 web toolset，不能仅写 config 而不向模型注册工具。Firecrawl 服务地址是
集群内非敏感配置；若固定上游版本要求 API key，则使用 namespace 内 Secret，由所有 app Pod 以只读
env 注入同一服务凭证，绝不写入 manifest、日志或前端。
