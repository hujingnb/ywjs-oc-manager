...
API、worker、Playwright 必须分别配置 HPA，按 CPU 与内存阈值在表中上下限之间自动扩缩；后续若集群
提供队列 external metrics adapter，可在不改变接口的前提下为 worker 增加队列深度指标。Redis、RabbitMQ、
PostgreSQL 是单副本临时依赖，不作伪高可用声明。所有 Deployment 使用 `Recreate` 或安全的单副本策略，
数据库不挂 PVC、不接 S3。任一基础依赖重启会丢失排队与执行中的任务；Hermes 将该次 `web_extract` 视为
失败并正常告知信息无法读取，不得编造正文或来源。
