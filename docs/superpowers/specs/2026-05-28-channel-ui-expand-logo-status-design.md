...
- logo 来源：海外渠道（Telegram/WhatsApp/Discord/Slack/Line/WeChat）优先取 Iconify 的
  `logos` 集（gilbarbara/logos，全彩品牌 logo，打包 MIT；注意 simple-icons 为单色，不采用）；
  中文渠道（企业微信、飞书/Lark、钉钉）在通用集中覆盖不全，从各自开放平台官方 brand 资源页取
  官方全彩 SVG。取到后将 SVG 源码**内联**进 `ChannelLogo.vue`，最终产物为本地内联、零远程依赖、
  零运行时 npm 依赖，便于单元测试稳定断言，也避免远程加载失败影响页面。品牌 logo 作指代渠道接入
  的指代性使用，按官方 / MIT 集合取用即可。
- 全彩仅对已支持渠道生效：当前仅微信需要真正全彩，其余 8 个均经 `grayscale(1)` 压为灰度，
  全彩源与单色源肉眼几乎无差。本设计仍按全彩 SVG 入库，便于将来某渠道转 `supported: true` 时
  直接可用。
