...
页面设计已通过可视化 companion 确认。整体后台采用“运维控制台式”布局，成员的“我的应用”页面吸收“应用中心式”的详情体验。

### 4.1 整体布局

- 使用左侧固定主导航 + 顶部状态栏 + 主内容区。
- 左侧导航承载当前角色可访问的一级模块；同一套壳适配平台管理员、组织管理员、组织成员。
- 顶部状态栏展示当前角色、当前组织、运行节点关键状态、全局错误或异步任务提示。
- 主内容区以高密度表格、状态摘要、Tabs、操作按钮和确认弹窗为主，不做营销式首页。
- 页面视觉风格应克制、清晰、适合长期运维；优先保证扫描效率、状态可读性和操作安全。

### 4.2 页面模板

列表页：

- 使用筛选区 + 数据表格 + 行内操作 + 批量操作。
- 筛选区支持关键字、状态、组织、runtime node、时间范围等按页面需要启用。
- 表格列优先展示状态、归属、节点、用量、最近更新时间和主要操作。
- 高风险操作必须使用确认弹窗，并在确认弹窗中展示影响范围。

详情页：

- 顶部展示资源名称、主状态、关键副状态和主要操作。
- 顶部状态摘要使用紧凑指标块，覆盖运行状态、渠道状态、节点状态、api_key 状态、用量或同步状态。
- 详情主体使用 Tabs：运行、渠道、知识库、工作目录、Job、审计等按资源类型启用。
- 异步状态必须可见，支持手动刷新和按需轮询。

创建成员 + 应用页：

- 使用单页分组表单，不使用多步向导。
- 分组包括账号信息、应用配置、Runtime Node、AI 人设、初始应用级知识库。
- 表单提交前展示即将创建的成员、应用、渠道绑定和初始化 job 摘要。
- 提交成功后跳转应用详情页，展示初始化 job 状态。

工作目录页：

- 使用只读文件管理器模式。
- 顶部展示面包屑、当前路径、下载当前目录按钮。
- 文件列表展示名称、类型、大小、修改时间和下载操作。
- 不提供上传、删除、重命名、在线编辑入口。
- 下载和打包下载失败必须展示明确错误，并写审计。

渠道绑定页：

- 使用应用详情内 Tab，不单独做复杂向导。
- 按 `AuthChallenge.type` 渲染认证组件；第一版 `qr_code` 显示二维码、过期时间、刷新/重试操作。
- 绑定状态、失败原因和下一步动作必须在同一视图中可见。

### 4.3 角色页面结构

平台管理员：

- 总览、组织管理、组织充值、全平台应用、Runtime Node、全平台用量、平台管理员账号、审计日志。
- 总览优先展示组织数、应用数、运行容器数、异常应用、节点异常、Token Credit 和近期失败 job。

组织管理员：

- 组织总览、成员账号、创建成员 + 应用、组织应用、组织 AI 人设、组织知识库、组织用量、组织审计。
- 成员和应用页面优先支持快速筛选异常状态、绑定状态、节点状态和知识库同步状态。

组织成员：

- 我的总览、我的应用、渠道绑定、应用知识库、组织知识库只读、工作目录、我的用量、个人设置。
- “我的应用”页面采用应用中心式详情：应用状态摘要常驻，运行、渠道、知识库、工作目录和用量通过 Tabs 切换。

### 4.4 浏览器验收

- 每个新增页面必须通过 chrome-devtools MCP 打开验证。
- 验证内容包括页面加载、导航可达、筛选和表单可用、关键按钮可点击、确认弹窗展示、异步状态刷新、错误提示可见。
- 需要截图或 DOM 快照确认主要文本不重叠、状态标签清晰、表格在常见桌面宽度下可读。
- 页面实现偏离本章节设计时，必须先更新规格并重新确认，再继续实现。
