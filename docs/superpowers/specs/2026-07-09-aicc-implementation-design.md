...
管理后台新增企业「概览」页作为企业管理员登录后的默认落点。概览页展示企业状态、常用运营数据和「子系统入口」区域；仅当企业已开通 AICC 时，该区域展示「AICC 客服」卡片。点击卡片跳转到 `/aicc-console` 独立工作台。

AICC 管理端不放入现有左侧菜单，不嵌入实例管理页面，也不复用主后台的内容布局。`/aicc-console` 仍复用现有登录态、组织权限、`aicc_enabled` 开通校验和 AICC API，但拥有独立顶部栏、AICC 专属导航和面向客服运营的页面结构。平台管理员在企业管理页维护 `aicc_enabled` 和 `aicc_agent_limit`。

企业管理员 AICC 工作台包含：

1. 接待台：展示今日会话、待处理线索、智能体状态和关键运营提醒，作为 AICC 子系统首页。
2. 会话：按智能体、时间、解决状态和关键词筛选，详情展示消息、图片、来源、地域、留资和反馈。
3. 知识库：配置机器人使用的行业知识库、企业知识库和专属文档入口。
4. 线索：线索列表、未读标记、去重提示和 CSV 导出。
5. 分析：会话趋势、热门问题、未解决率、地域、来源页和留资转化。
6. 设置：智能体列表和配置、投放配置、独立链接、二维码、网页挂件代码、允许域名和深度样式配置。

样式方向：

- 企业概览页采用主后台控制台风格，重点承载企业状态与子系统入口。
- AICC 工作台采用独立运营控制台风格，信息密度高，便于日常看会话、线索和转化。
