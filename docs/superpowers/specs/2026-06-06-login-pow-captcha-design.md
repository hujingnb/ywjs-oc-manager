...
```
                   登录请求 POST /api/v1/auth/login
                              │
   ┌──────────────────────────▼──────────────────────────┐
   │  Altcha PoW 校验（常驻，captcha.enabled 时执行）       │
...
type Verifier struct {
    hmacKey   []byte        // captcha.hmac_secret
    maxNumber int64         // captcha.difficulty
    ttl       time.Duration // captcha.ttl
}
...
- `captcha.enabled=false` → 返回 **204 No Content**（前端据此不渲染 widget、不禁用按钮，kill-switch 单一真相源在后端）；
...
> **故意不加 `binding:"required"`**——是否必填由 `captcha.enabled` 在 `Login()` 里判，
...
### 5.8 Config（YAML 内嵌，非环境变量）

本项目配置不走环境变量，而是一份 YAML（`OCM_CONFIG` 指向，内嵌在 k8s
Secret 的 `manager.yaml` 字段里），由 `gopkg.in/yaml.v3` 解析进
`internal/config` 的 `Config` 结构体。新增配置按既有模式办：

**① `internal/config/config.go` — 新增 `CaptchaConfig` 并挂到 `Config`：**

```go
type CaptchaConfig struct {
    Enabled    bool     `yaml:"enabled"`     // 总开关
    HMACSecret string   `yaml:"hmac_secret"` // 出题/验签 HMAC 密钥（敏感）
    Difficulty int64    `yaml:"difficulty"`  // altcha maxnumber，常驻取低值≈几百 ms
    TTL        Duration `yaml:"ttl"`         // 挑战有效期 = 一次性 key 最长 TTL，复用既有 Duration 类型
}
// Config 增加： Captcha CaptchaConfig `yaml:"captcha"`
```

**② `internal/config/loader.go` `Validate()` — 启用时才必填校验：**
`Captcha.Enabled==true` 时，`hmac_secret` 非空、`difficulty>0`、`ttl>0`，
否则 fail-fast；关闭时全部可缺省。

**③ YAML 形态（manager.yaml 内嵌段）：**

```yaml
captcha:
  enabled: false            # 默认关，先合代码后灰度
  hmac_secret: "<32+ 字节强随机>"
  difficulty: 50000         # 常驻起点，约几百 ms
  ttl: "5m"
```

**④ 配置文件同步清单（含 prod，用户明确要求同步线上）：**

| 文件 | 是否入 git | 改什么 |
|---|---|---|
| `internal/config/config.go` | ✅ | 加 `CaptchaConfig` 结构体 + `Config.Captcha` 字段 |
| `internal/config/loader.go` | ✅ | `Validate()` 加启用时必填校验 |
| `cmd/server/main.go` | ✅ | 装配 `pow.Verifier` + `ReplayGuard` 注入 `auth_service` |
| `deploy/k8s/local/secret.yaml` | ✅ | 内嵌 `manager.yaml` 加 `captcha:` 段（本地可 `enabled: true` + 低 `difficulty` 便于联调，dev 密钥可入库） |
| `deploy/k8s/prod/secret.example.yaml` | ✅ | 内嵌 `manager.yaml` 加 `captcha:` 段，`hmac_secret: "__FILL_captcha.hmac_secret__"` 占位 |
| `deploy/k8s/prod/secret.yaml` | ❌ **被 .gitignore** | 填真实强随机 `hmac_secret`，**不提交**；改完用 `make update-config`（`kubectl apply` + `rollout restart manager-api`）生效 |

> **prod 密钥安全**：`deploy/k8s/prod/secret.yaml` 已被 `.gitignore`（第 73 行）
> 忽略，真实 `hmac_secret` 用强随机生成、只落在线上 Secret，绝不入 git——
> 满足 CLAUDE.md「不提交密钥」。占位符走 committed 的 `secret.example.yaml`。
> 上线顺序：先 `enabled: false` 合代码 → prod `secret.yaml` 填好真实密钥并
> `make update-config` → 单一环境验证 → 改 `enabled: true` 再 `make update-config`。
...
- `captcha.enabled=false`（challenge 返回 204）时**不渲染 widget、不禁用按钮**。
...
Redis 挂时**跳过一次性消费，仅保留验签（HMAC + 过期）**：登录仍可用；代价是故障窗口内丧失防重放（攻击者可在 `captcha.ttl` 内重放同一个解）。验签层仍要求每个挑战真算 PoW，且 Argon2id 密码层兜底。仅记 `Warn` 日志，不向用户暴露错误。

### 7.2 kill-switch 一致性

`captcha.enabled` 为后端单一真相源：关闭时 `/altcha-challenge` 返回 204，前端据此不渲染 widget、不禁用按钮——后端热关验证码即可同步前端，无需重新发版前端。挑战接口临时报错时 widget 显示重试、按钮保持禁用，靠 kill-switch 快速止血。

### 7.3 灰度

`captcha.enabled` 默认 `false` 先合代码，再在单一环境开启验证，通过后才到生产。
...
- `captcha.enabled=false`（challenge 204）时不渲染 widget、按钮直接可用。
...
- 跑相关后端 / 前端单测；
- **配置文件同步**（用户明确要求）：本地 `deploy/k8s/local/secret.yaml`、prod 模板 `deploy/k8s/prod/secret.example.yaml` 都已加 `captcha:` 段并随代码提交；prod 真实 `deploy/k8s/prod/secret.yaml`（gitignore）填好强随机 `hmac_secret`、`make update-config` 已生效（先 `enabled: false` 上线，验证后再灰度开启）；
- **真实浏览器**全流程验证（不能用 curl 替代）：
  - 打开登录页 → widget 自动转圈 → 几百 ms 后打勾、按钮启用；
  - 正确凭证 → 登录成功跳转；
  - 错误密码 → 提示「账号或密码错误」+ widget 自动重算，可再试；
  - 关掉 `captcha.enabled` → 登录页无 widget、按钮直接可用、登录正常（验 kill-switch）；
