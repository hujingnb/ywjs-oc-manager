...
| `runtime/hermes/hermes-aicc/{oc-entrypoint.py,entrypoint_helpers.py,renderer/*}`、`tests/test_entrypoint_integration.py`、`test_render_*.py` | 无状态幂等启动、临时渲染、Skill/配置裁剪 | AICC-BOOT-002..004、AICC-CAP-002 | `PYTHONPATH=runtime/hermes/hermes-aicc pytest -q runtime/hermes/hermes-aicc/tests` | 已映射；端到端 Pod 重建仍以矩阵 BLOCKED 为准。`lib/atomic.py` 是未改动依赖，范围外。 |
...
| `cmd/server/main.go`、`internal/worker/aicc/message_dispatch_loop.go` 及测试 | 本地失败控制面、异步消息、意向重试与任务租约 | AICC-INT-001..003、AICC-BOOT-001 | `go test ./internal/worker/aicc -run AICC -count=1` | 局部测试通过；Chrome 验收 BLOCKED 见矩阵 |
