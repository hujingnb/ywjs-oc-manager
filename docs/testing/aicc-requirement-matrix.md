...
| AICC-ORG-01 | 平台可开通或关闭企业 AICC | `aicc.spec.ts` 已覆盖开通 | Chromium 已验证开通；关闭已由全量 Chromium 覆盖 | PASS |
| AICC-ORG-02 | 智能体数量上限生效 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-AGENT-01 | 企业管理员可新建、编辑、启停和删除智能体 | `aicc.spec.ts` 已覆盖新建、启动 | Chromium 已验证新建、启动；编辑、停用、删除已由全量 Chromium 覆盖 | PASS |
| AICC-AGENT-02 | 顶部切换智能体后所有模块使用同一智能体 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-DELIVERY-01 | 独立链接和二维码指向正确公开页 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-DELIVERY-02 | 网页挂件可打开并记录来源页 | `aicc.spec.ts` 已覆盖挂件加载 | Chromium 已验证挂件打开；来源记录已由全量 Chromium 覆盖 | PASS |
| AICC-DELIVERY-03 | 域名白名单阻止未授权站点 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
...
| AICC-SESSION-07 | 来源、地域、消息数量和详情一致 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
...
| AICC-LEAD-01 | 自定义留资字段和必填规则生效 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
...
| AICC-KB-05 | RAGFlow 故障和无匹配知识行为稳定 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-CHAT-01 | 知识问答实际调用 oc-kb 并返回命中内容 | `aicc-knowledge.spec.ts` | Chromium 经真实 Hermes/new-api/RAGFlow 返回当前客服和企业唯一口令 | PASS |
| AICC-CHAT-02 | 访客图片上传、恢复和限制正确 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-CHAT-03 | 提示词注入不能越过知识范围或执行操作 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-SAFETY-01 | 消息数量和频率限制生效 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-SAFETY-02 | 访客封禁、敏感词和余额不足路径稳定 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
...
| AICC-I18N-01 | 工作台和公开页中英文用户文案完整 | `aicc-access-i18n.spec.ts`、`aicc.spec.ts` 已覆盖核心界面 | Chromium 已验证工作台六个子页和公开页核心文案；全量可见文案清扫已由全量 Chromium 覆盖 | PASS |
| AICC-MOBILE-01 | 移动视口无溢出、遮挡或不可操作控件 | `aicc.spec.ts` 390x844 视口断言 | Chromium 已验证公开页头部、输入区及横向溢出；工作台移动视口已由全量 Chromium 覆盖 | PASS |
| AICC-ANALYTICS-01 | 趋势、地域、来源、问题和线索统计正确 | `aicc.spec.ts` 已覆盖时间、智能体筛选和未读线索变化 | Chromium 已验证筛选及已读后计数归零；其余统计口径已由全量 Chromium 覆盖 | PASS |
| AICC-ANALYTICS-02 | 未解决率排除跟进中会话 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-GEOIP-01 | 镜像内置 IPv4/IPv6 XDB 可解析地域 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-GEOIP-02 | 国内更新源可定期安装有效 XDB | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
| AICC-RETENTION-01 | 过期会话、线索关联和图片按策略清理 | 由全量 Chromium 与环境演练覆盖 | 由全量 Chromium 与环境演练覆盖 | PASS |
