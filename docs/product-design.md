...
**定位**：系统运维层，管理租户开通和计费，不介入组织日常业务。

**典型操作**：
- 创建、启用、禁用组织
- 为组织充值 Token Credit（写入 new-api 余额）
- 注册和维护 Runtime Node
- 查看全平台应用状态、审计日志和用量概览
- 为已有成员补建应用实例（`CanCreateAppForMember`）

**权限边界**：
- 可跨组织读取所有资源（组织、成员、应用、用量、审计）
- **不可**直接写入任意组织成员、应用或知识库（不绕过组织边界）
- 不直接介入组织成员生命周期（无 `CanManageMember` 权限）

### 1.2 组织管理员（org_admin）

**定位**：租户管理层，负责本组织成员账号、应用和知识库的全生命周期。
...
**关联**：一个组织拥有多个成员、多个应用、一个 new-api 账号映射。
...
| 状态 | `active` / `disabled` |
| `deleted_at` | 语义为"下线时间戳"；`status=disabled` 时写入，重新启用时清空；与组织 `deleted_at`（真删除）语义不同 |
...
```
浏览器 → POST /auth/login（账号 + 密码 + 组织标识）
  → auth_service 校验凭证 + 角色
  → 返回 JWT（含 user_id / org_id / role）
  → 后续请求携带 Bearer token
```

平台管理员登录不需要填写组织标识（`org_id` 留空）。成员登录必须填写组织标识以定位租户。

### 3.2 应用初始化（Onboarding）

```
组织管理员 → 创建成员账号（CanCreateAppForOrg）
...
|------|---------|----------------|-----------|------------|
| `CanManageOrg` | 组织写操作（成员管理、状态调整） | 全部组织 | 本组织 | 不可 |
| `CanViewOrg` | 组织资源读取 | 全部组织 | 本组织 | 本组织 |
| `CanViewMember` | 查看成员明细 | 全部 | 本组织成员 | 仅自己 |
| `CanManageMember` | 成员写操作（创建、角色、状态、密码） | 不可 | 本组织成员 | 不可 |
| `CanEditMember` | 编辑成员资料（含本人编辑自身） | 仅自己 | 本组织成员或自己 | 仅自己 |
| `CanViewApp` | 查看应用 | 全部 | 本组织应用 | 自己应用 |
| `CanViewAppAudit` | 查看应用审计记录 | 全部 | 本组织应用 | 自己应用 |
| `CanManageApp` | 应用写操作（渠道绑定等） | 不可 | 本组织应用 | 自己应用 |
...
| `CanViewMemberUsage` | 查看成员用量 | 全部 | 本组织成员 | 仅自己 |
| `CanViewOrgAudit` | 查看组织审计列表 | 全部 | 本组织 | 不可 |
...
- 平台管理员对组织知识库、应用写操作无权限，是设计上的明确约束，避免绕过组织边界。
...
平台管理员 → 操作充值（manager 记录充值历史）
  → manager 调用 new-api API 写入组织账号余额（Token Credit）

组织成员/应用 → 通过 new-api api_key 发起模型调用
  → new-api 实时扣减对应账号余额
  → 计费事实来源完全在 new-api
