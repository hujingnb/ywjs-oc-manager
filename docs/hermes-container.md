...
`ywjs-26257ea5.ecis.huabei-3.cmecloud.cn/app/oc-manager-aigowork:v2026.5.16-2026-05-21-12-00-00`
这种时间戳 tag。镜像版本锁通过
...
├── skills/
│   ├── .bundled_manifest            # [oc-entrypoint] 镜像内置 skill 基线
...
| `/opt/data/.oc-state.json` | oc-entrypoint | `lib/state.py` | pod 重建前从 S3 恢复，容器 start 时读写 |
| `/opt/data/skills/oc-kb/SKILL.md` | oc-entrypoint | `renderer/render_skills.py` | 容器 start 时重渲染 |
| `/opt/data/skills/.bundled_manifest` | oc-entrypoint | `oc_entrypoint.ensure_builtin_manifest` | pod 重建前从 S3 恢复；缺失时首次启动生成 |
...
| `/opt/data/cron/` | Hermes / oc-ops | Hermes cron / `ocops.cron` | 不动，pod 重建前从 S3 恢复 |
| `/opt/data/logs/` | Hermes | Hermes 进程 | 不动 |
| `/opt/data/kanban.db` | Hermes | Hermes 进程 | 不动，pod 重建前从 S3 恢复 |
