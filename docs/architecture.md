...
### 2.4 Hermes 容器（runtime/hermes/hermes-v2026.5.16/）

`runtime/hermes/` 下每个子目录是独立 runtime variant。当前 variant 是
`runtime/hermes/hermes-v2026.5.16/`，其
`runtime/hermes/hermes-v2026.5.16/Dockerfile` 与同目录 `healthcheck.sh`、
`CONTRACT.md`、`renderer/`、`migrator/`、`lib/`、入口脚本一起定义容器行为。
每个应用对应节点上的一个 Hermes 容器，由 `app_initialize` job 通过 agent
docker proxy 创建。

挂载约定：manager 将中立输入写到节点 `{data_root}/apps/<app_id>/input/`，
