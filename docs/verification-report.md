...
承接 `docs/superpowers/plans/2026-05-04-v1-rc-finalization-implementation-plan.md` Chunk-2。
...
承接 `docs/superpowers/plans/2026-05-04-v1-rc-finalization-implementation-plan.md` Chunk-3。
...
- **Task 12 平台总览页 `PlatformDashboardPage.vue`** 尚未做（spec §5.4 要求组织数 / 成员数 / 应用数 / 运行中容器数 / 异常应用数 / 总余额 / 总用量七项汇总）；当前 platform_admin 已可在 `/usage` 平台 tab 看跨组织用量，剩缺 dashboard 入口。后续随 Chunk-4 终检一并补齐。
...
- Task 11 自动重启实测尚未做"docker exec ocm-{app_id} kill 1"破坏性测试；当前仅验证 60s 周期成功探针写入。后续 Chunk-3 收尾或 Chunk-4 验证时补做。
