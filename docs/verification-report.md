...
| 检查项 | 结果 | 备注 |
|---|---|---|
| `已废弃的双 agent compose override` | ✅ | override 文件，跑 `docker compose 旧双 agent override 命令（现已废弃） up -d` 同时起两个 agent；7003/7004 端口 + 独立 .local/data/agent-2 目录 |
...
`docker compose 旧双 agent override 命令（现已废弃）` 起两 agent。
...
2. **双 agent 默认共享 `./config/agent.yaml`**：导致 agent token 互相覆盖。本次给
   ...
   两节点都通过 agent token 验证。`已废弃的双 agent compose override` 后续应增加
...
- `已废弃的双 agent compose override` 加独立 yaml mount 让 agent token 不互相覆盖。
...
- `已废弃的双 agent compose override`：agent-2 mount 独立 `.local/data/agent-2-config:/app/config`。
...
| force-recreate agent-2 | `docker compose -f ... -f 已废弃的双 agent compose override up -d --force-recreate oc-runtime-agent-2` | 8 秒内第一次心跳到达 manager |
...
docker compose 旧双 agent override 命令（现已废弃） up -d --force-recreate manager-api oc-runtime-agent oc-runtime-agent-2
