...
日志配置在 `manager.yaml` 的 `logging` 段（即 secret 内嵌的 `manager.yaml`，见
`deploy/k8s/{local,prod}/secret.yaml`），结构见 `internal/config.LoggingConfig`。整段可缺省，
loader 回填默认（info / json / 200ms），改动经 `make prod-update-config`（线上）或 `make local-build`（本地）生效：

```yaml
logging:
  level: "info"        # debug / info / warn / error，默认 info；生产排故可临时调 debug 后改回
  format: "json"       # json（默认，容器/ELK）| text（本地调试人眼友好）
  slow_query_ms: 200   # SQL 慢查询阈值（毫秒），默认 200；耗时超过即从 Debug 抬到 Warn
```

- `level` / `format` 非法值各自回退（info / json）；`slow_query_ms` 为 0 或负值视为未配置，回退 200。
