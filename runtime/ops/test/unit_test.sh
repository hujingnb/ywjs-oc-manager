...
BOOTSTRAP_REAL_JQ=$(command -v jq)
...
fetch_bootstrap() {
  ...
  cat > "\$1" <<'JSON'
{"manifest_yaml":"MANIFEST_SENTINEL","persona":"PERSONA_SENTINEL","platform_rule":"PLATFORM_RULE_SENTINEL","skills":[{"name":"blocked-skill","url":"https://example.invalid/skill.tar"}],"s3_write":{"bucket":"must-not-be-read","prefix":"forbidden/","access_key_id":"AK","secret_access_key":"SK"}}
JSON
}
EOF
# mock curl/aws/s3/head-object 只记录并失败；命令一旦被调用，断言即可发现 bootstrap
# 错误下载 skill 或访问对象存储（包含绕过 aws 的独立子命令调用）。
...
exit 99
EOF
cat > "$BOOTSTRAP_BIN/s3" <<EOF
#!/usr/bin/env bash
printf 's3 %s\\n' "\$*" >> "$BOOTSTRAP_CALLS"
exit 99
EOF
cat > "$BOOTSTRAP_BIN/head-object" <<EOF
#!/usr/bin/env bash
printf 'head-object %s\\n' "\$*" >> "$BOOTSTRAP_CALLS"
exit 99
EOF
# jq 包装层记录 JSON 查询后转调真实命令，确保启动命令未解析被禁止的 bootstrap 字段。
cat > "$BOOTSTRAP_BIN/jq" <<EOF
#!/usr/bin/env bash
printf 'jq %s\\n' "\$*" >> "$BOOTSTRAP_CALLS"
exec "$BOOTSTRAP_REAL_JQ" "\$@"
EOF
chmod +x "$BOOTSTRAP_BIN/curl" "$BOOTSTRAP_BIN/aws" "$BOOTSTRAP_BIN/s3" "$BOOTSTRAP_BIN/head-object" "$BOOTSTRAP_BIN/jq"
...
# 三个互异 sentinel 必须分别写入对应文件，防止字段错误映射仍因文件非空而漏检。
assert_eq "$(cat "$BOOTSTRAP_INPUT/manifest.yaml")" "MANIFEST_SENTINEL" "oc-bootstrap 应精确写入 manifest_yaml"
assert_eq "$(cat "$BOOTSTRAP_INPUT/resources/persona.md")" "PERSONA_SENTINEL" "oc-bootstrap 应精确写入 persona"
assert_eq "$(cat "$BOOTSTRAP_INPUT/resources/platform-rules.md")" "PLATFORM_RULE_SENTINEL" "oc-bootstrap 应精确写入 platform_rule"
...
# jq 查询只能读取三个启动字段；skills、restore、s3_write 都属于本命令的隔离边界。
if grep -Eq '^jq .*(\.skills|\.restore|\.s3_write)' "$BOOTSTRAP_CALLS"; then echo "FAIL: oc-bootstrap 不得解析 skills/restore/s3_write"; fail=1; fi
rm -rf "$BOOTSTRAP_TMP"
