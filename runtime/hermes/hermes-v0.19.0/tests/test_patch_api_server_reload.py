...
# v0.19.0 上游把 connect() 内的逐条 add_post 改成 _http_route_table()
# 返回路由表，再由 connect() 统一 add_route；补丁必须适配该结构。
_FAKE_API_SERVER_ROUTE_TABLE = (
    "class APIServerAdapter:\n"
    "\n"
...
def test_patch_injects_route_table_entry_for_v019_api_server():
    # v0.19.0 场景：路由在 _http_route_table() 中以 tuple 声明。
    result = _mod.patch(_FAKE_API_SERVER_ROUTE_TABLE)
    assert (
        '("POST", "/oc/skills/reload", self._handle_oc_skills_reload),'
        in result
    )
    assert result.index("/oc/skills/reload") > result.index(
        "/v1/runs/{run_id}/stop"
    )
