...
REPLACEMENTS_RUN: list[tuple[str, str]] = [
    ...
    # 危险命令文本审批标题：v0.19.0 上游抽到 _format_exec_approval_fallback helper。
    # smart_denied 分支保留上游专用文案；普通危险命令标题接入 catalog。
    (
    ...
    # 危险命令审批原因标签：v0.19.0 helper 使用 description 参数。
    (
        'f"{heading}\\n```\\n{cmd_preview}\\n```\\nReason: {description}\\n\\n"',
        'f"{heading}\\n```\\n{cmd_preview}\\n```\\n" + '
        't("oc.run.confirm_reason", desc=description)',
    ),
