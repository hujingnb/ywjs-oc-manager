...
# v0.19.0 上游把路由集中到 _http_route_table() 的 tuple 列表中，再由
# connect() 统一 add_route 并镜像到 /p/{profile} 前缀；在这里插入 tuple，
# 可继承上游 profile multiplex 行为。
ROUTE_TABLE_ANCHOR = (
    '            ("POST", "/v1/runs/{run_id}/stop", self._handle_stop_run),\n'
)
ROUTE_TABLE_INJECT = (
    '            # OC 扩展路由（oc-manager 注入）：供 oc-ops 触发免重启 skill 热加载\n'
    '            ("POST", "/oc/skills/reload", self._handle_oc_skills_reload),\n'
)
...
def patch(content: str) -> str:
    ...
    has_legacy_route_anchor = ROUTE_ANCHOR in content
    has_route_table_anchor = ROUTE_TABLE_ANCHOR in content
    if not has_legacy_route_anchor and not has_route_table_anchor:
        raise RuntimeError(
    ...
    # 追加路由注册：v0.19.0 走 _http_route_table()，旧结构走 connect() 内 add_post。
    if has_route_table_anchor:
        content = content.replace(
            ROUTE_TABLE_ANCHOR, ROUTE_TABLE_ANCHOR + ROUTE_TABLE_INJECT
        )
    else:
