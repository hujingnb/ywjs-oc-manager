...
def test_no_prev_skips(tmp_data: Path) -> None:
    ...
    result = run_migration(prev_variant=None, curr_variant="hermes-v2026.5.15", data_root=tmp_data)
...
def test_same_variant_skips(tmp_data: Path) -> None:
    ...
    result = run_migration(
        prev_variant="hermes-v2026.5.15",
        curr_variant="hermes-v2026.5.15",
        data_root=tmp_data,
    )
    assert result is None
...
def test_migration_module_suffix_replaces_dash_and_dot() -> None:
    ...
    assert _migration_module_suffix("hermes-v2026.5.15") == "hermes_v2026_5_15"
