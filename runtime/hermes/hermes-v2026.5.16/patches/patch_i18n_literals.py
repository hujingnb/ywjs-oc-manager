...
REPLACEMENTS_RUN: list[tuple[str, str]] = [
    ...
    # --- run_agent 异常回包里的 status_hint（按 HTTP status_code 给出提示，
    #     最终拼进发往对话的 "Sorry, I encountered an error (...)" 消息）。
    #     v2026.6.5 把这套逻辑重构成了独立的 _gateway_provider_error_reply
    #     辅助函数（措辞也变了），5.16 仍是这批内联 status_hint 字符串。
    #     状态码 401/402/429(plan limit/限流)/529 各一条；400 的
    #     " The request was rejected by the API." 已在下方通用兜底覆盖。---
    (" Check your API key or run `claude /login` to refresh OAuth credentials.",
     " 请检查你的 API key,或运行 `claude /login` 刷新 OAuth 凭证。"),
    (" Your API balance or quota is exhausted. Check your provider dashboard.",
     " 你的 API 余额或配额已用尽。请检查服务商控制台。"),
    (" Your plan's usage limit has been reached. It resets in ~{_hours}h.",
     " 你的套餐用量上限已达到。约 {_hours} 小时后重置。"),
    ...
    # --- 上下文压缩告警（5.16 经 _adapter.send 直发对话框；6.5 改了措辞）---
    # 摘要生成失败：丢弃的历史轮次不可恢复（多物理段 f-string，{占位符}原样保留）。
    ("⚠️ Context compression summary failed ",
     "⚠️ 上下文压缩摘要失败 "),
    ("({_err}). {_dropped} historical message(s) ",
     "({_err})。{_dropped} 条历史消息"),
    ...
    ("Consider /reset for a clean session, or check ",
     "建议用 /reset 开始干净会话,或检查"),
    ("your auxiliary.compression model configuration.",
     "你的 auxiliary.compression 模型配置。"),
    # 配置的辅助压缩模型失败、已回退主模型：提示用户检查 config.yaml。
    ...
    # ====================== 5.16 专属（与 6.5 文案不同）======================
    # 长任务心跳（5.16 文案；6.5 为「⏳ Working — N min」）
    ("⏳ Still working... ({_elapsed_mins} min elapsed",
     "⏳ 仍在处理... (已过 {_elapsed_mins} 分钟"),
    # 心跳 _parts 的 running 段（5.16 独有；6.5 直接用 str(_action) 无字面量）
    ("running: {_a['current_tool']}", "运行中: {_a['current_tool']}"),
    # 语音 STT 安装命令（5.16 用 pip）
    ("(`pip install faster-whisper` in the Hermes venv) ",
     "(在 Hermes venv 中执行 `pip install faster-whisper`) "),
]
