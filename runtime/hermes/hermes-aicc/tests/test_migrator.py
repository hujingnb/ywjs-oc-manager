...
def test_no_prev_skips(tmp_data: Path) -> None:
    ...
    result = run_migration(prev_variant=None, curr_variant="hermes-v2026.7.1", data_root=tmp_data)
...
def test_same_variant_skips(tmp_data: Path) -> None:
    ...
    result = run_migration(
        prev_variant="hermes-v2026.7.1",
        curr_variant="hermes-v2026.7.1",
        data_root=tmp_data,
    )
    assert result is None
