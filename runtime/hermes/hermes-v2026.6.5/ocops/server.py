...
async def feishu_register(request):
    ...
    query 可带 domain（feishu|lark），默认 feishu。鉴权由 AuthMiddleware 统一处理。
    feishu_register 是 async generator，逐条转 SSE data 帧；所有错误已在 generator 内
    降级为 failed 事件，handler 层无需额外 try/except。
    """
