...
def hermes_json(args: list[str]):
    ...
    hermes-v2026.6.5 v0.14.0 已确认：某些业务错误（如 show 不存在的任务）以退出码 0 +
...
def hermes_ok(args: list[str]) -> None:
    ...
    业务错误（hermes-v2026.6.5 v0.14.0 已确认某些错误如此返回）。当前所有写 verb 在
...
def parse_watch_line(line: str):
    ...
    hermes-v2026.6.5 v0.14.0 已确认：kanban watch 子命令无 --json 选项，输出
...
def run_capabilities() -> dict:
    ...
    return {
        ...
        "variant": info.get("variant") or info.get("oc_image_variant") or "hermes-v2026.6.5",
