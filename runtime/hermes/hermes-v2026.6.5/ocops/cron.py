...
def _advanced_job_updates(args) -> dict:
    """提取 manager 支持但 Hermes v2026.6.5 CLI 未直接暴露的 per-job 字段。"""
    updates = {}
...
def _create_args(args) -> list[str]:
    """适配 Hermes v2026.6.5：create 的 schedule/prompt 是位置参数。"""
...
def _update_args(args) -> list[str]:
    """适配 Hermes v2026.6.5：edit 的 job_id 是位置参数，其余字段仍用 flag。"""
