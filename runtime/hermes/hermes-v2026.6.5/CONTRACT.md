...
- 数据迁移: 本 variant 由上一版 `hermes-v2026.5.16` 整体复制并升级 version.txt 而来。跨 variant 数据过渡由 `migrator/` 基于文件检测完成（按 data_root 自身 `.oc-state.json` 记录的版本判断，而非按来源版本名 dispatch），对升级 / 降级、任意来源版本通用，不需要为 `hermes-v2026.5.16` 单独写 `from_hermes_v2026_5_16.py` 迁移模块。当前各 variant 的 on-disk 布局一致，从 `hermes-v2026.5.16` 切换到本 variant 为 no-op 过渡。
